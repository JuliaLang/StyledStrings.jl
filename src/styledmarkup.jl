...
function resolveface(state::State, facename::String)
    ...
    elseif state.strict
        ...
        lookmakeface(state.out.mod, Symbol(facename))
    end
end
...
function addannot!(state::State, i::Int, label, value)
    ...
    for (a, annot) in Iterators.reverse(enumerate(state.out.annots))
        ...
        if annot.label == styannot.label && annot.value == styannot.value && annot.loff == styannot.loff
...
macro styled_str(raw_content::String)
    ...
    lexp = Expr(:let, Expr(:block), Expr(:block, state.out.lets..., aexp))
    if iszero(state.interpcount)
        hygienic_eval(state, lexp)
    else
        lexp
    end
end
