...
Base.getindex(ti::TermInfo, key::Symbol) = ti.capabilities[key]
Base.get(ti::TermInfo, key::Symbol, default::D) where D<:Union{Bool, Int, String} =
    get(ti.capabilities, key, default)::D
Base.get(ti::TermInfo, key::Symbol, default) = get(ti.capabilities, key, default)
Base.keys(ti::TermInfo) = keys(ti.capabilities)
Base.haskey(ti::TermInfo, key::Symbol) = haskey(ti.capabilities, key)
...
@static if VERSION < v"1.8"
    current_terminfo = TermInfo()
else
    current_terminfo::TermInfo = TermInfo()
end

# Legacy/Base.TTY methods and the `:color` parameter
