...
@testset "SimpleColor" begin
    ...
    @test sprint(show, SimpleColor(face"blue")) |> choppkg ==
        "SimpleColor(face\"blue\")"
    @test sprint(show, SimpleColor(0x123456)) |> choppkg ==
        "SimpleColor(0x123456)"
    @test sprint(show, MIME("text/plain"), SimpleColor(face"blue")) |> choppkg ==
        "SimpleColor(blue)"
    @test sprint(show, MIME("text/plain"), SimpleColor(face"blue"), context = :color => true) |> choppkg ==
...
@testset "Faces" begin
    ...
    let aface = hacky_addface!(:a, Face(font="a", height=1.2))
        bface = hacky_addface!(:b, Face(font="b", height=1.1, weight=:light, inherit=:a))
        cface = hacky_addface!(:c, Face(font="c", foreground=face"red", inherit=:b))
        dface = hacky_addface!(:d, Face(font="d", foreground=face"blue", weight=:bold))
        eface = hacky_addface!(:e, Face(font="e", inherit = [:c, :d]))
        fface = hacky_addface!(:f, Face(font="f", inherit = [:d, :c]))
    ...
    @test sprint(show, MIME("text/plain"), getface(), context = :color => true) |> choppkg ==
        ...
        Face \e[1mdefault\e[22m (sample)
         ...
         strikethrough: false
               inverse: false\
        """
    @test sprint(show, MIME("text/plain"), face"red", context = :color => true) |> choppkg ==
        """
        Face \e[1mred\e[22m (\e[31msample\e[39m)
            foreground: \e[31m■\e[39m red\
        """
    @test sprint(show, face"red") |> choppkg == "face\"red\""
    @test sprint(show, copy(face"red")) |> choppkg ==
        "Face(foreground = face\"red\")"
    @test sprint(show, MIME("text/plain"), copy(face"red"), context = :compact => true) |> choppkg ==
        "Face(foreground = face\"red\")"
    @test sprint(show, MIME("text/plain"), copy(face"red"), context = (:compact => true, :color => true)) |> choppkg ==
        "Face(foreground = face\"\e[31mred\e[39m\")"
    @test sprint(show, MIME("text/plain"), copy(face"highlight"), context = :compact => true) |> choppkg ==
        "Face(inverse = true, inherit = [face\"emphasis\"])"
    with_terminfo(vt100) do # Not truecolor capable
        @test sprint(show, MIME("text/plain"), copy(face"region"), context = :color => true) |> choppkg ==
            """
    ...
    with_terminfo(vt100) do # Ensure `enter_reverse_mode` exists
        @test sprint(show, MIME("text/plain"), copy(face"highlight"), context = :color => true) |> choppkg ==
            """
...
@testset "Styled Markup" begin
    ...
    @test styled"{(inherit=bface):a}" == AnnotatedString("a", [(1:1, :face, Face(inherit=bface))])
    @test styled"{(inherit=[aface,bface]):a}" == AnnotatedString("a", [(1:1, :face, Face(inherit=[aface, bface]))])
